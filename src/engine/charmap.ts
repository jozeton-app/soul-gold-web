...
const CHARMAP: Record<number, string> = {
  0x00: ' ', 0xAB: '!', 0xAC: '?', 0xAD: '.', 0xAE: '-', 0xB5: '♂', 0xB6: '♀', 0xB8: ',', 0xBA: '/',
...
const INV_CHARMAP: Record<string, number> = {};
Object.entries(CHARMAP).forEach(([k, v]) => {
  if (v) INV_CHARMAP[v] = parseInt(k, 10);
});
INV_CHARMAP[' '] = 0x00;

export function decodeGbaString(bytes: Uint8Array, maxLength: number = 8): string {
  let result = '';
  for (let i = 0; i < bytes.length && i < maxLength; i++) {
    const b = bytes[i];
    if (b === 0xFF || b === 0x00 && i > 0) break;
    result += CHARMAP[b] || '?';
  }
  return result.trim();
}

export function encodeGbaString(text: string, maxLength: number = 8): Uint8Array {
  const buf = new Uint8Array(maxLength);
  buf.fill(0xFF);
  for (let i = 0; i < maxLength; i++) {
    if (i < text.length) {
      const char = text[i];
      buf[i] = INV_CHARMAP[char] !== undefined ? INV_CHARMAP[char] : 0x00;
    }
  }
  return buf;
}